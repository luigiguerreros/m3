 ...
 * on the project 'base' directory, then on the M3 base directory.
 *
...
class FileSearch
{
    ...
    // Busca el fichero dentro de la carpeta de M3, apps y base
    public $search_m3 = true;
    ...
    private function search()
    {
        ...
        // La app donde buscar.
        $app = $this->search_app ? $this->search_app : M3::$application;
        
        // Armamos las rutas de búsqueda
        if ($app) {
            $this->searched_paths = [
                join(M3\PROJECT_PATH, 'apps', $app, $this->type, $this->file),
            ];
        }
        ...
        if ($this->search_base) {
            $this->searched_paths[] = 
                join(M3\PROJECT_PATH , 'base', $this->type, $this->file);
        ...
        if ($this->search_m3) {
            if ($app) {
                $this->searched_paths[] = 
                    join(M3\BASE_PATH, 'apps', $app, $this->type, $this->file );
            }

            $this->searched_paths[] =             
                join(M3\BASE_PATH, 'base', $this->type, $this->file );
        }
