...
class Dropdown extends ControlAbstract
{
	...
	function draw() {
		...
		$variables["%{$this->ajax_search_variable}"] = "m3.id('{$this->search_field}').value";

		$variables = m3\json ( $variables );
...
{$dropdown_name} = dropdown.create ( '{$this->name}', '{$this->search_field}', function(accept) {
    ajax.{$this->ajax_method} ( '{$this->ajax_url}', {$variables}, function(ajax_data){
        if (accept) {
        	...
        	if ( id ) {
	        	m3.id ('{$this->name}').value = id
	        	m3.id ('{$this->search_field}').value = data[id]
	        }
	        else {
	        	// Sin resultados
	        	m3.id ('{$this->name}').value = ''
	        	m3.id ('{$this->search_field}').value = ''
	        }

	        // Execute any callback
	        if ( m3.id({$dropdown_name}) && m3.id({$dropdown_name}).close_callback )
	            m3.id({$dropdown_name}).close_callback()
