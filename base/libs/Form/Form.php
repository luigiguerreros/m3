...
class Form 
{
    ...
    private $properties = [
        ...
        'html' => [
            ...
            'message_block' => ['div', 'class' => 'm3_form_messageblock'],
         ...
         */
        'html_prefix_messageblock_id' => 'm3_form_messages_',
    ...
    public function __construct(CsrfInterface $csrf, $id = null) 
    {
        ...
        foreach ($props as $prop) {
            ...
            if (!class_exists ($class_name)) {
                throw new M3\Exception("Form field class '$class_name' unknow.");
            }
    ...
    /*public function __construct(array $controls, $csrf_token, $id = null) 
    {
        ...
        foreach ($controls as $control => $fielddef) {
            ...
            if (!class_exists ($class)) {
                throw new M3\Exception ( "Form field class '$class' unknow for '$name'.");
            }
     ...
     *
     * @param array $values Associative array with the control's values.
     */
    public function setValues(array $values)
    {
        // Veriricamos si $values es un model
        if (is_object($values)) {
            if ($values instanceof M3\AsArrayInterface) {
                $values = $values -> asArray();
            }
            throw new \RuntimeException("Object can't be used as form values.");
        }

        // El campo __M3_TOKEN es especial
        $key = '__M3_CSRF_TOKEN';
    ...
    public function draw(...$control_list)
    {
        ...
        foreach ($control_list as $cname) {
            ...
            $label = '';
            if ($control->draw_label) {
                $label = $control->drawLabel();
            }

            // Verificamos los tags HTML para encerrar a cada parte
            foreach(['label','info', 'widget'] as $type) {
