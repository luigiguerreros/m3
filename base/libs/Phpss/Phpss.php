...
/**
 * PHPSS, a PHP CSS optimizer for M3. OMG, right? #sheldoncooper.
 *
...
class phpss {
    ...
    function at_import ( $node ) {
        ...
        // Ubicamos el fichero
        $path = new M3\Path\FileSearch($file, "assets/css", 'css');
...
//                }
    ...
    function __construct () {
        ...
        foreach ( $files as $f ) {
            ...
            $path = new M3\Path\FileSearch($f, "assets/css" );
            $path->ext = 'css';

            // Si no existe, explotamos.
            if ( $path->notFound() ) {
                throw new M3\Exception ( "CSS Asset '$f' not found", [
