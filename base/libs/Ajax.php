...
class Ajax
{
    ...
    {
        if (M3::$request->getHeaderLine('X-M3-Requested-With') 
            == 'XmlHttpRequest' 
            && 
            M3::$request->getHeaderLine('X-M3-Security-Token') 
    ...
    public static function process($method = null, $callback = null)
    {
        ...
        M3::$execution_type = 'ajax';

        if (self::verify() && M3::$request->getMethod() == $method) {
